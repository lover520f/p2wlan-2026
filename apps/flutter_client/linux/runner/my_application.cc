...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "P2WLAN");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "P2WLAN");
  }

  gtk_window_set_default_size(window, 1100, 720);
  gtk_widget_set_size_request(GTK_WIDGET(window), 860, 560);
  ...
  // for transparent.
  gdk_rgba_parse(&background_color, "#F6F8FA");
