...
class ParallelRoomPlan {
  ParallelRoomPlan(AppSettings account, this.room) {
    ...
    final port = 40000 + int.parse(profileId.substring(0, 8), radix: 16) % 20000;
...
class ParallelRooms extends ChangeNotifier {
  ...
  }) : supported = supported ?? (Platform.isWindows || Platform.isMacOS || Platform.isLinux) {
  ...
  bool get hasSessions => _sessions.isNotEmpty || _operations.isNotEmpty || _recovering.isNotEmpty;
  bool get busy => _operations.isNotEmpty || _stoppingAll != null;
  ParallelRoomSession? session(String roomId) => _sessions[roomId];

  String _credentialKey(AppSettings value) => '${value.controlServer}\n${value.authToken}';
  ...
  Future<DaemonCommandResult> connect(FriendRoom room) {
    ...
    if (_disposed || _stoppingAll != null) return Future.value(_fail('网络正在关闭，未启动房间'));
    if (_credentialKey(readSettings()) != _credentials) {
      return credentialsChanged().then((result) => result.ok ? connect(room) : result);
    }
    final epoch = _epoch;
  ...
  bool _accepts(int epoch, String credentials) =>
      !_disposed && _stoppingAll == null && epoch == _epoch &&
      credentials == _credentialKey(readSettings());

  Future<DaemonCommandResult> _connect(FriendRoom room, int epoch, String credentials) async {
    if (!_accepts(epoch, credentials)) return _fail('登录状态已变化，已取消连接');
    if (_sessions.values.any((entry) =>
        _credentialKey(entry.plan.settings) != credentials)) {
    ...
    if (_sessions.length >= maxConnections) return _fail('已达到并行房间上限 $maxConnections');
    ...
    if (!started.ok || !_accepts(epoch, credentials)) {
      ...
      return cleanup.ok ? (started.ok ? _fail('登录状态已变化，连接已取消') : started) : cleanup;
    }
  ...
  String? _conflict(ParallelRoomPlan plan, AppSettings account) {
    ...
    if (cidrsOverlap(account.overlayCidr, plan.room.cidr)) return '房间网段与主网络配置重叠';
    if (Uri.tryParse(account.diagnosticsUrl)?.port == Uri.parse(plan.settings.diagnosticsUrl).port) {
  ...
  Future<DaemonCommandResult> stopAll() {
    ...
    unawaited(() async {
      try {
        ...
        final results = await Future.wait(_sessions.keys.toList().map(_disconnect));
        final failed = results.any((result) => !result.ok);
        final forced = results.any((result) => result.forcedTermination);
        completer.complete(DaemonCommandResult(
          ok: !failed,
          message: failed ? '部分房间未能停止，请重试；尚未清除运行时记录' : '所有并行房间已停止',
          graceful: !failed && !forced && results.every((result) => result.graceful),
  ...
  Future<void> recoverJoinedRooms() async {
    if (!supported || _disposed || _stoppingAll != null) return;
    final account = readSettings();
  ...
  Future<void> recover(FriendRoom room) async {
    if (!supported || _disposed || _stoppingAll != null ||
        _sessions.containsKey(room.id) || !_recovering.add(room.id)) return;
    try {
      await _enqueue(room.id, () async {
        if (_sessions.containsKey(room.id) || _stoppingAll != null) return _ok();
        final account = readSettings();
        final plan = ParallelRoomPlan(account, room);
        if (_conflict(plan, account) != null || _sessions.length >= maxConnections) return _ok();
        ...
        if (_credentialKey(account) != _credentialKey(readSettings()) || _stoppingAll != null) {
  ...
  Future<void> _refresh(ParallelRoomSession entry) async {
    ...
    try {
      ...
      if (_disposed || revision != entry.revision ||
          !identical(_sessions[entry.plan.room.id], entry)) return;
      ...
      if (!_disposed && revision == entry.revision && identical(_sessions[entry.plan.room.id], entry)) {
        entry.snapshot = null;
  ...
  static DaemonCommandResult _ok() => const DaemonCommandResult(ok: true, message: '完成', graceful: true);
  static DaemonCommandResult _fail(String message) => DaemonCommandResult(ok: false, message: message);
}
...
bool cidrsOverlap(String left, String right) {
  (int, int)? range(String text) {
    ...
    if (ip == null || ip.type != InternetAddressType.IPv4 || prefix == null || prefix < 0 || prefix > 32) return null;
    final address = ip.rawAddress.fold<int>(0, (value, byte) => (value << 8) | byte);
